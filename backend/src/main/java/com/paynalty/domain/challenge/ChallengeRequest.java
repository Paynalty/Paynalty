...
public class ChallengeRequest {
        ...
        @Schema(description = "챌린지 시작 날짜 (필수)\n" +
                        "- 형식: YYYY-MM-DD (예: 2025-01-01)\n" +
                        "- 오늘 날짜 이후여야 함 (오늘 포함 불가)\n" +
                        "- 프론트에서 계산된 날짜를 전달\n" +
                        "  * 내일 시작: 오늘 기준 +1일\n" +
                        "  * 다음주 시작: 오늘 기준 다음주 월요일", example = "2026-02-25", required = true)
        @NotNull(message = "시작 날짜는 필수입니다")
        private LocalDate startDate;

        @Schema(description = "챌린지 종료 날짜 (필수)\n" +
                        "- 형식: YYYY-MM-DD (예: 2025-12-31)\n" +
                        "- 오늘 날짜보다 이후여야 함 (오늘 포함 불가)\n" +
                        "- 시작일보다 이후여야 함", example = "2026-12-31", required = true)
        @NotNull(message = "종료 날짜는 필수입니다")
        private LocalDate endDate;

        @Schema(description = "인증 주기 (주에 몇 번 인증할지)\n" +
                        "⚠️ designatedDays와 frequency는 둘 중 하나만 사용 가능\n" +
                        "- designatedDays가 null이거나 빈 배열일 때만 사용\n" +
                        "- designatedDays가 있으면 이 값은 무시되고 자동 계산됨\n" +
                        "- 예: 4 (요일 상관없이 주에 4회 인증)", example = "null")
        private Integer frequency;

        @Schema(description = "지정된 요일 목록\n" +
                        ...
                        "- 가능한 요일 값: MON, TUE, WED, THU, FRI, SAT, SUN", example = "[\"MON\", \"TUE\", \"WED\", \"THU\", \"FRI\", \"SAT\", \"SUN\"]", allowableValues = {
        ...
        @Schema(description = "인증 가능 시작 시간 (선택)\n" +
                        "- 형식: HH:mm (예: 06:00)\n" +
                        "- null이면 기본값 00:00 사용\n" +
                        "- verifyEndAt보다 이전이어야 함", example = "00:00:00")
        private LocalTime verifyStartAt;

        @Schema(description = "인증 가능 종료 시간 (선택)\n" +
                        "- 형식: HH:mm (예: 23:59)\n" +
                        "- null이면 기본값 23:59 사용\n" +
                        "- verifyStartAt보다 이후여야 함", example = "23:59:59")
        private LocalTime verifyEndAt;

        @Schema(description = "인증 타입 (필수)\n" +
                        "- PHOTO: 사진 인증\n" +
                        "- TEXT: 텍스트 인증\n" +
                        "- VOTE: 투표 인증", example = "PHOTO", required = true, allowableValues = { "PHOTO", "TEXT",
                                        "VOTE" })
        @NotNull(message = "인증 타입은 필수입니다")
        private VerificationType verificationType;

        @Schema(description = "초대할 친구 목록\n" +
                        "- 이름과 전화번호를 포함하는 객체 리스트\n" +
                        "- 예: [{\"name\": \"홍길동\", \"phoneNumber\": \"010-1111-2222\"}, {\"name\": \"이순신\", \"phoneNumber\": \"010-3333-9999\"}]", example = "[2,3,7]")
