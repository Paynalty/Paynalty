package com.paynalty.domain.test;
...
public class TestController {
    ...
    public ResponseEntity<String> generateDummyData(@AuthenticationPrincipal CustomUserDetails userDetails) {
        ...
        createVerifications(c2, allUsers, today);
        ...
        createVerifications(c3, allUsers, today);
        ...
        createVerifications(c4, allUsers, today);
        ...
        createVerificationsForCompleted(c5, allUsers);
        ...
        createVerifications(c6, allUsers, today);
    ...
    private void createVerifications(Challenge challenge, List<User> allUsers, LocalDate today) {
        // 시작일부터 어제까지의 인증 데이터
        for (LocalDate date = challenge.getStartDate(); date.isBefore(today); date = date.plusDays(1)) {
            final LocalDate d = date;
            // 해당 요일이 챌린지 요일에 포함되는지 확인 (단순화를 위해 일단 생성)
            allUsers.stream().limit(3).forEach(u -> challengeVerificationRepository.save(ChallengeVerification.builder()
                    .user(u)
                    .challenge(challenge)
                    .date(d)
    ...
    private void createVerificationsForCompleted(Challenge challenge, List<User> allUsers) {
        for (LocalDate date = challenge.getStartDate(); !date.isAfter(challenge.getEndDate()); date = date.plusDays(1)) {
            final LocalDate d = date;
            allUsers.stream().limit(2).forEach(u -> challengeVerificationRepository.save(ChallengeVerification.builder()
                    .user(u)
                    .challenge(challenge)
                    .date(d)
                    .imageUrl("https://static.toss.im/ml-product/observer-binocular.png")
